...
public class RunPiles
{
 public static void main(String[] arg) {
   ...
   int[] ss = {11}; //init state

   PilesSearch psearch = new PilesSearch(); //the search engine
   GameSearchState initState = (GameSearchState) new PilesState(ss); //initial state - 1 pile of 10
   String ans = psearch.runGameSearch(initState,"depth_first"); //run the search
   scr.println(ans);
  }
}
